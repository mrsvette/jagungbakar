...
class IDefaultController extends EController
{
	public static $_alias='Manage Inquiry';
	...
	public function actionView()
	{
		...
		if(Yii::app()->request->isAjaxRequest && isset($_GET['ModInquiry'])){
			foreach($_GET['ModInquiry'] as $attr1=>$val1){
				if(!empty($val1))
					$criteria->compare($attr1,$val1,true);
			}
		}

		$dataProvider=new CActiveDataProvider('ModInquiry',array('criteria'=>$criteria,'pagination'=>array('pageSize'=>5)));
	...
	public function actionCreate()
	{
		if(Yii::app()->request->isPostRequest)
		{
			...
			if($exc){
				...
				echo CJSON::encode(array(
					'status'=>'failed',
					'div'=>$this->renderPartial('_form_inquiry',array('model'=>$model),true,true),
				));
			}
			exit;
		}
	...
	{
		$model=ModInquiry::model()->findByPk($id);
		if($model===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $model;
	}
