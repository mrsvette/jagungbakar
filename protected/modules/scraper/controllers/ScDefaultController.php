...
class ScDefaultController extends EController
{
	public static $_alias = 'Manage Scraping';
	...
	public function actionView()
	{
		...
		if(Yii::app()->request->isAjaxRequest && isset($_GET['ModScraper'])){
			foreach($_GET['ModScraper'] as $attr1=>$val1){
				if(!empty($val1))
					$criteria->compare($attr1,$val1,true);
			}
		}

		$dataProvider = new CActiveDataProvider('ModScraper',array('criteria'=>$criteria,'pagination'=>array('pageSize'=>5)));
	...
	public function actionCreate()
	{
		if(Yii::app()->request->isPostRequest)
		{
			...
			$model = new ModScraper('create');
			$model->attributes = $_POST['ModScraper'];
			//default path uploads/images/gallery
			$path = Extension::getConfigByModule('scrapper','scrapper_upload_path');
			$model->src = $path.'/';
			...
			if($model->save()){
				...
				if(!empty($file)){
					...
					if($file->saveAs($path.'/'.$fname)){ //upload image
						...
						$force_resize = (int)Extension::getConfigByModule('scrapper','scrapper_image_force_resize');
						if($force_resize>0){
							$ideal_width = (int)Extension::getConfigByModule('scrapper','scrapper_image_width');
							$ideal_height = (int)Extension::getConfigByModule('scrapper','scrapper_image_height');
				...
				echo CJSON::encode(array(
					'status'=>'failed',
					'div'=>$this->renderPartial('_form_scrapper',array('model'=>$model),true,true),
				));
			}
			exit;
		}
	...
	public function actionUpdate($id)
	{
		if(Yii::app()->request->isPostRequest)
		{
			...
			$model->attributes = $_POST['ModScraper'];
			//default path uploads/images/scrapper
			$path = Extension::getConfigByModule('scrapper','scrapper_upload_path');
			$model->src = $path.'/';
			//default path uploads/images/scrapper/_thumbs
			$path_thumbs = $path.'/_thumbs';
			$model->thumb = $path_thumbs.'/';
	...
	{
		$model = ModScraper::model()->findByPk($id);
		if($model===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $model;
	}
