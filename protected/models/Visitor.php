...
/**
 * This is the model class for table "{{app_visitor}}".
 *
...
class Visitor extends CActiveRecord
{
	...
	public function getGraphPerMinute()
	{
		$data1=Yii::app()->db->createCommand(
				...
				FROM app_visitor WHERE DATEDIFF(NOW(),date_entry)=:diff');
	...
	public function getCountActivePage($url)
	{
		...
		//$sql='SELECT v.ip_address FROM app_visitor v WHERE v.url=:url AND DATEDIFF(NOW(),v.date_entry)=:diff GROUP BY v.ip_address';
		$sql='SELECT v.session_id FROM app_visitor v WHERE v.url=:url AND DATEDIFF(NOW(),v.date_entry)=:diff AND TIMEDIFF(NOW(),v.date_expired)<=:timediff ORDER BY id DESC';
		$command=Yii::app()->db->createCommand($sql);
	...
	{
		$sql='SELECT COUNT(v.mobile) as count FROM app_visitor v WHERE DATEDIFF(NOW(),v.date_entry)=:diff AND TIMEDIFF(NOW(),v.date_expired)<=:timediff AND mobile=:mobile AND v.active=:active ORDER BY id DESC';
		$command=Yii::app()->db->createCommand($sql);
	...
	public function getCountVisitor($date,$type='pageview')
	{
		switch($type){
			case 'pageview':
				$sql='SELECT COUNT(v.id) as count FROM app_visitor v WHERE DATE_FORMAT(v.date_entry,"%Y-%m-%d")=:date ORDER BY v.id DESC';
				$command=Yii::app()->db->createCommand($sql);
				...
				break;
			case 'session':
				$sql='SELECT COUNT(DISTINCT v.session_id) as count FROM app_visitor v WHERE DATE_FORMAT(v.date_entry,"%Y-%m-%d")=:date GROUP BY v.session_id';
				$command=Yii::app()->db->createCommand($sql);
				$command->params=array('date'=>$date);
			...
			case 'pageviewmonthly':
				$sql='SELECT COUNT(v.id) as count FROM app_visitor v WHERE DATE_FORMAT(v.date_entry,"%Y-%m")=:date ORDER BY v.id DESC';
				$command=Yii::app()->db->createCommand($sql);
				...
				break;
			case 'sessionmonthly':
				$sql='SELECT COUNT(DISTINCT v.session_id) as count FROM app_visitor v WHERE DATE_FORMAT(v.date_entry,"%Y-%m")=:date GROUP BY v.session_id';
				$command=Yii::app()->db->createCommand($sql);
				$command->params=array('date'=>date("Y-m",strtotime($date)));
				$row=$command->queryAll();
				return count($row);
	...
	public function getAverageUsers()
	{
		$date_from=date('Y-m-d', strtotime('last month'));
		$date_to=date('Y-m-d');
		$sql='SELECT COUNT(DISTINCT v.session_id) as count FROM app_visitor v WHERE DATE_FORMAT(v.date_entry,"%Y-%m-%d")>=:date_from AND DATE_FORMAT(v.date_entry,"%Y-%m-%d")<=:date_to GROUP BY DATE_FORMAT(v.date_entry,"%Y-%m-%d")';
		$command=Yii::app()->db->createCommand($sql);
	...
	public function getAveragePageView()
	{
		$date_from=date('Y-m-d', strtotime('last month'));
		$date_to=date('Y-m-d');
		$sql='SELECT COUNT(v.session_id) as count FROM app_visitor v WHERE DATE_FORMAT(v.date_entry,"%Y-%m-%d")>=:date_from AND DATE_FORMAT(v.date_entry,"%Y-%m-%d")<=:date_to GROUP BY DATE_FORMAT(v.date_entry,"%Y-%m-%d")';
		$command=Yii::app()->db->createCommand($sql);
		$command->params=array('date_from'=>$date_from,'date_to'=>$date_to);
		$rows=$command->queryAll();
		if(count($rows)>0){
	...
	{
		$sql='SELECT COUNT(DISTINCT v.ip_address) as count FROM app_visitor v';
		$command=Yii::app()->db->createCommand($sql);
		$row=$command->queryRow();
		return $row['count'];
	}
	...
	public function getAverageDuration()
	{
		...
		$date_to=date('Y-m-d');
		$sql='SELECT DISTINCT v.session_id FROM app_visitor v WHERE DATE_FORMAT(v.date_entry,"%Y-%m-%d")>=:date_from AND DATE_FORMAT(v.date_entry,"%Y-%m-%d")<=:date_to GROUP BY v.session_id';
		$command=Yii::app()->db->createCommand($sql);
		$command->params=array('date_from'=>$date_from,'date_to'=>$date_to);
		$rows=$command->queryAll();
		$items=array();
		if(count($rows)>0){
			foreach($rows as $row){
				$sql2='SELECT TIMEDIFF(MAX(v.date_entry),MIN(v.date_entry)) as diff FROM app_visitor v WHERE v.session_id=:session_id';
