...
class Menu extends CActiveRecord
{
	...
	public function items($group,$level_from=1,$linkOptions=array())
	{
		...
		foreach($models as $model){
			if(self::issetChild($model->id))
				$list[$model->level_id][$model->id]=array('label'=>Yii::t('menu',$model->content_rel->nama_menu), 'url'=>'#', 'items'=>self::child($model->id),'visible'=>$model->getVisibility(),'linkOptions'=>$linkOptions);
			else{
				if($model->content_rel->link_action=='#')
					$list[$model->level_id][$model->id]=array('label'=>Yii::t('menu',$model->content_rel->nama_menu), 'url'=>array('/#'),'visible'=>$model->getVisibility(),'linkOptions'=>$linkOptions);
				else
					$list[$model->level_id][$model->id]=array('label'=>Yii::t('menu',$model->content_rel->nama_menu), 'url'=>self::getLinkAction($model),'visible'=>$model->getVisibility(),'linkOptions'=>$linkOptions);
			}
		}
	...
	public function child($menu_id)
	{
		...
		foreach($models as $model){
			if(self::issetChild($model->id))
				$items[]=array('label'=>Yii::t('menu',$model->content_rel->nama_menu), 'items'=>self::child($model->id),'visible'=>$model->getVisibility());
			else{
				if($model->content_rel->link_action=='#')
					$items[]=array('label'=>Yii::t('menu',$model->content_rel->nama_menu), 'url'=>array('/#'),'visible'=>$model->getVisibility());
				else
					$items[]=array('label'=>Yii::t('menu',$model->content_rel->nama_menu), 'url'=>self::getLinkAction($model),'visible'=>$model->getVisibility());
			}	
		}
		return $items;
	}
