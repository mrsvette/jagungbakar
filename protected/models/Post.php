...
class Post extends CActiveRecord
{
	...
	{
		$criteria=new CDbCriteria;
		$criteria->compare('LOWER(slug)',strtolower($title));
		$criteria->with=array('content_rel');
		$model=self::model()->find($criteria);
		return (empty($model->id))? false : $model->content_rel->content;
	}
	...
	public function getLayoutItems($title='- Pilih Layout -')
	{
		$columns=glob('themes/'.Yii::app()->theme->name.'/views/layouts/column*.php');
		$items=array();
		if(!empty($title))
			$items['']=$title;
		if(is_array($columns)){
			foreach($columns as $index=>$column){
				$name=basename($column,'.php');
				$items[$name]=$name;
			}
		}
		return $items;
